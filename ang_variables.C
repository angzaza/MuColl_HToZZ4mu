...
// for(unsigned int ientry=0; ientry<5000; ++ientry){
 for(unsigned int ientry=0; ientry<c_sig->GetEntries(); ++ientry){
         ...
         if(rcmu.Pt()>pt_cut){
           ...
           if(abs(trk_d0[tr_fts[rc_ftr[i_rc]]])<d0_cut){
             ...
             if(abs(trk_z0[tr_fts[rc_ftr[i_rc]]])<z0_cut){
               counter_z0_mu++;
               count_z0_mu_evt++;		
               // muon TLorentz vectors will be stored in two different arrays according to the charge sign
               if(rc_cha[i_rc]>0){
   ...
   if(condition_4mu_sel==1){
     ...
     delta_min_2=500.;
     pt_max=-1.;
     //in general we have count_mup_evt positive muons and count_mum_evt negative muons,  with count_mup_evt>=2 and count_mum_evt>=2
     //in order to reconstruct the ZZ candidates, 4 muon combinations are constructed with the following cycles in i,j,k,l
