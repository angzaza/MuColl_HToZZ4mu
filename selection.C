...
// declaration of file paths
  ...
  ofstream myfile;
  myfile.open ("selection_output_bkg_3tev_200k_ordered.txt");
...
// VARIABLES DECLARATION
  ...
  int counter_6dR=0, counter_pt10=0, counter_pt20=0;
  int condition_4mu_sel=0;
  int condition_dR=0, condition_pt=0, condition_mass_4mucomb=0;
  int condition_Z=0, condition_Z1=0, condition_ZZ=0, condition_mass_4mu=0;
  int counter_h=0;
  int counter_dR=0, counter_pt=0, counter_4mu_mass=0;
...
// VARIABLES FROM BRANCHES
  ...
  TH1F* invM_Z1=new TH1F("Z1mass_bkg","Z1mass_bkg",220,10,120);
  TH1F* invM_Z2=new TH1F("Z2mass_bkg","Z2mass_bkg",220,10,120);
  TH1F* invM_H=new TH1F("Hmass_bkg","Hmass_bkg",475,50,1000);
  TH1F* invM_H_zoom=new TH1F("Hmass_zoom_bkg","Hmass_zoom_bkg",120,100,160);
  TH1F* invM_H_cut=new TH1F("Hmass_sig_aftercut","Hmass_sig_aftercut",120,100,160);
  TH1F* pt_H_histo=new TH1F("Pt_higss_bkg","Pt_higgs_bkg",500,0,500);
			...
			for (int j=0;j<count_mum_evt;j++){
		  	for(int k=i;k<count_mup_evt;k++){
		   		for (int l=0;l<count_mum_evt;l++){	
		   			if(k!=i && l!=j){	
		   				counter_6dR=0; counter_pt10=0; counter_pt20=0;
		   				condition_dR=0; condition_pt=0; condition_mass_4mucomb=0;
     					...
     					for(int a=0;a<2;a++){
   						...
   						if(((mum_sel[0]+mum_sel[1]+mup_sel[0]+mup_sel[1]).M())>70){
   								condition_mass_4mucomb=1; //condition_mass_4mu is switched to 1 if the invariant mass of the selected 4 muons is larger than 70 GeV
   						}
   								...
   								if(condition_dR==1){
   									...
   									if(condition_pt==1){
   										...
   										if(inv_mass[za_index]>inv_mass_Z1_cut){
   											...
   											if(condition_mass_4mucomb==1){
   												condition_mass_4mu=1;
   						...
   						if(condition_mass_4mu==1){
   									...
   									if(fabs(inv_mass[z1_index]-mass_z)<delta_min_2){
   										...
   										z1_vec=mum[(int)(z1_index/2)]+mup[(int)(z1_index%2)];
   										z2_vec=mum[(int)(z2_index/2)]+mup[(int)(z2_index%2)];
   										pt_max=pt_sum_z2;
                                                                                for(int i_mu=0;i_mu<2;i_mu++){
                                                                                  if(mum_sel[i_mu].Pt()>=5 && mum_sel[i_mu].Pt()<20){
                                                                                    bibw_muon[i_mu]=w1;
                                                                                  }
                                                                                  if(mum_sel[i_mu].Pt()>=20 && mum_sel[i_mu].Pt()<30){
                                                                                    bibw_muon[i_mu]=w2;
                                                                                  }
                                                                                  if(mum_sel[i_mu].Pt()>=30 && mum_sel[i_mu].Pt()<40){
                                                                                    bibw_muon[i_mu]=w3;
                                                                                  }
                                                                                  if(mum_sel[i_mu].Pt()>=40 && mum_sel[i_mu].Pt()<60){
                                                                                    bibw_muon[i_mu]=w4;
                                                                                  }
                                                                                  if(mum_sel[i_mu].Pt()>=60 && mum_sel[i_mu].Pt()<100){
                                                                                    bibw_muon[i_mu]=w5;
                                                                                  }
                                                                                  if(mum_sel[i_mu].Pt()>=100 && mum_sel[i_mu].Pt()<200){
                                                                                    bibw_muon[i_mu]=w6;
                                                                                  }
                                                                                  if(mup_sel[i_mu].Pt()>=5 && mup_sel[i_mu].Pt()<20){
                                                                                    bibw_muon[i_mu+2]=w1;
                                                                                  }
                                                                                  if(mup_sel[i_mu].Pt()>=20 && mup_sel[i_mu].Pt()<30){
                                                                                    bibw_muon[i_mu+2]=w2;
                                                                                  }
                                                                                  if(mup_sel[i_mu].Pt()>=30 && mup_sel[i_mu].Pt()<40){
                                                                                    bibw_muon[i_mu+2]=w3;
                                                                                  }
                                                                                  if(mup_sel[i_mu].Pt()>=40 && mup_sel[i_mu].Pt()<60){
                                                                                    bibw_muon[i_mu+2]=w4;
                                                                                  }
                                                                                  if(mup_sel[i_mu].Pt()>=60 && mup_sel[i_mu].Pt()<100){
                                                                                    bibw_muon[i_mu+2]=w5;
                                                                                  }
                                                                                  if(mup_sel[i_mu].Pt()>=100 && mup_sel[i_mu].Pt()<200){
                                                                                    bibw_muon[i_mu+2]=w6;
                                                                                  }

                                                                                }
                                                                                bibw_event=bibw_muon[0]*bibw_muon[1]*bibw_muon[2]*bibw_muon[3];
...
myfile<<"Higgs mass between 105 and 140 GeV: "<<counter_h<<endl;
 myfile<<"number of weighted events: "<<n_wevts<<endl;
myfile<<"number of weighted events in the Higgs region: "<<nH_wevts<<endl;


myfile.close();

TFile *rootFile = new TFile("Selection_bkg_3tev_200k_ordered.root","RECREATE");
invM_Z1->Write();
invM_Z2->Write();
invM_H->Write();
invM_H_zoom->Write();
